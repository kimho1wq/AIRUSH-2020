...
class ResNetSE_SA(nn.Module):
    def __init__(self, block, layers, num_filters, nOut, encoder_type='SAP', **kwargs):
        ...
        super(ResNetSE_SA, self).__init__()
    ...
    def forward(self, x, nb_frame_SA = 50, softmax=False):
        ...
        l_x = []
        nb_time = x.size(-1)
        window_size = nb_frame_SA // 10
        if nb_time > nb_frame_SA:
            step = nb_frame_SA - window_size
            iter = (nb_time - window_size) // step + 1
            for i in range(iter):
                if i == 0:
                    l_x.append(x[:, :, :, :nb_frame_SA])
                elif i < iter-1:
                    l_x.append(x[:, :, :, i*step : i*step+nb_frame_SA])
                else:
                    l_x.append(x[:, :, :, -nb_frame_SA:])
        else:
            iter = 1
        ...
        for i in range(iter):
            x = l_x[i]
            #instancenorm.size torch.Size([200, 1, 40, nb_frame_SA ?])
            ...
            x = x.view(x.size()[0], -1)
            x = self.fc(x)
            if i==0: s_code = x.view(x.size(0), -1, 1)
            else: s_code = torch.cat([s_code, x.view(x.size(0), -1, 1)], dim=2)

        x = torch.mean(s_code, dim=2).view(x.size(0), -1)
        return x
...
def ResNetSE34L(nOut=256, **kwargs):
    ...
    model = ResNetSE_SA(SEBasicBlock, [3, 4, 6, 3], num_filters, nOut, **kwargs)
    return model

def ResNetSEL_16k_150():
    """Model2_16k's configuration
    sr : 16000
    nOut : 512
    max_frames : 150
    encoder_type : SAP
    """
    model = ResNetSE34L(nOut=512, max_frames=150, encoder_type="SAP", sr=16000)
